    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((s) => (
          <div
            key={s.id}
            className={`group rounded-2xl border bg-gradient-to-br ${CATEGORY_COLORS[s.category] || CATEGORY_COLORS.other} overflow-hidden transition hover:scale-[1.02]`}
          >
            {/* Icon header */}
            <div className="p-6 pb-3">
              <div className="flex items-start justify-between">
                <span className="text-3xl">{CATEGORY_ICONS[s.category] || "⚙️"}</span>
              ...
              <div>
                {user?.is_vip && user.vip_discount_percent > 0 ? (
                  <div className="flex items-center gap-2">
                    <span className="text-white/40 line-through text-sm">{s.price} د.ت</span>
                    <span className="text-lg font-bold text-amber-400">{(s.price * (100 - user.vip_discount_percent) / 100).toFixed(1)} د.ت</span>
                  </div>
                ) : (
                  <span className="text-lg font-bold text-white">{s.price} د.ت</span>
                )}
